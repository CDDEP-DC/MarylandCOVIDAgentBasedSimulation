...
def main():
    ...
    ###  data to fit
    MarylandFitData = {}
    MarylandFitData[datetime(2020,3,22)] = {'hospitalized':18,'admissions':64}
    MarylandFitData[datetime(2020,3,23)] = {'hospitalized':52,'admissions':98}
    ...
    for grun in range(0,1000):
        ...
        for nrun in range(0,NumberMeanRuns):
            ...
            for day in results.keys():
                ...
                x = datetime(2020, 3, 23) - timedelta(days=round(endTime,0)-day)
                if x >= datetime(2020, 3, 22) and x <= datetime(2020, 3, 23):
        ...
        for i in range(0,len(NewParameterVals)):
            ...
            mutvals1 = [1]*25
            mutvals2 = [1]*25
            for m in range(0,len(mutvals1)):
                if random.random()<.01:
                    mutvals1[m] = random.random()*(.95 - 1.05) + .95
                if random.random()<.02:
                    mutvals2[m] = random.random()*(.95 - 1.05) + .95
            Child1 = {
                'endTime':NewParameterVals[parent1]['endTime']*mutvals1[0] if random.random() < .5 else NewParameterVals[parent2]['endTime']*mutvals2[0], 
                ...
                'ImportationRate':NewParameterVals[parent1]['ImportationRate'] if random.random() < .5 else NewParameterVals[parent2]['ImportationRate'],
                'ImportationRatePower':NewParameterVals[parent1]['ImportationRatePower'] if random.random() < .5 else NewParameterVals[parent2]['ImportationRatePower'],
                'diffC':0
            }
